...
void historic(char *trozos[])
{
	...
	if (trozos[1] != NULL)
	{
		...
		else if (strncmp(trozos[1], "-r", 2) == 0)
		{ //Lo mismo que arriba
...
void infoAux(char *path)
{
	...
	if (lstat(path, &buf) == -1)
	{ //comprobamos que funcione lstat
	...
	{ //listamos la informacion
		...
		if (LetraTF(buf.st_mode) == 'l')
		{ //si es un link, lo muestra y a donde apunta
...
void listAux(char *path, bool op_long, bool op_dir, bool op_hid, bool op_rec)
{
	...
	if (d != NULL)
	{
		...
		while ((dir = readdir(d)) != NULL)
		{
			...
			if ((op_hid == true) || ((strcmp(dir->d_name, ".") != 0) && (strcmp(dir->d_name, "..") != 0)))
			{
				...
				{
					if (op_long == true)
					{ // si queremos listar con su informacion
						infoAux(aux);
					}
					else
					{
		...
		for (int j = 0; j < i; j++)
		{ //procesamos los directorios de la cola
			strcpy(aux, path);
			strcat(aux, "/");
			strcat(aux, cola[j]);
	...
	{ //si no hemos podido abrir el directorio
...
void create(char *trozos[])
{
	if (trozos[1] != NULL)
	{
		...
		{ //si la opcion es -dir
			if (trozos[2] != NULL)
			{
				if (mkdir(trozos[2], 0777) == -1)
				{ //comprobamos que se ha creado el directorio
					perror(trozos[2]);
				}
			}
			else
			{ //si no hay mas argumentos lista
				listAux(".", false, true, false, false);
			}
		}
		else
		{ //si no hay '-dir' creamos un archivo y comprobamos su creacion
...
void deleteAux(char *path)
{
	...
	if (d != NULL)
	{
		while ((dir = readdir(d)) != NULL)
		{
			...
			{ //si es un directorio borramos recursivamente
				deleteAux(aux);
				if (remove(aux) == -1)
			...
			else if ((dir->d_type == DT_REG) || (dir->d_type == DT_LNK))
			{ //si es un archivo o un link simbolico borramos directamente
				if (remove(aux) == -1)
				{ //comprobamos borrado
					perror(aux);
				}
...
void list(char *trozos[], int n)
{
	...
	if (n == comienzo)
	{ // si solo hay listar (sin opciones)
		listAux(".", op_long, op_dir, op_hid, op_rec);
	}
	else
	{
		for (int i = comienzo; i < n; i++)
		{ //listamos segun las opciones incluidas
...
void *MmapFichero(char *fichero, int protection)
{
	...
	strftime(((struct node *)aux)->t, 200, "%a \n", GetFecha());
